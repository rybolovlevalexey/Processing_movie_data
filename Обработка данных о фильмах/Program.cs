...
namespace ќбработка_данных_о_фильмах
{
    class Program
    {

        static Dictionary<string, Movie> films = new Dictionary<string, Movie>();  // название: фильм
        static Dictionary<string, List<Movie>> people = new Dictionary<string, List<Movie>>();  // им€ учатсника: фильмы
        static Dictionary<string, List<Movie>> tags_dict = new Dictionary<string, List<Movie>>();  // тэг: фильмы
        static string dataset_path = @"C:\”нивер\ml-latest\";
        ...
        {
            //Console.WriteLine("ќбработка данных о фильмах");
            ...
            // наполнение фильмов их актЄрами и режиссЄрами
            ...
            // финальные приготовлени€
            // наполнение второго словар€
            foreach (var per in result_people.Values)
            {
            ...
            // наполнение третьего словар€, когда все классы фильмов заполнены
            foreach (var film_name in result_tags.Keys.AsParallel())
            {
                ...
                switch (mode)
                {
                    case "a":
                        ...
                        if (!films.ContainsKey(movie_name))
                            Console.WriteLine("”казанный фильм не найден");
                        else
                        {
                            var result = films[movie_name];
                            Console.WriteLine($"‘ильм {result.name} с рейтингом {result.rating}");
                            Console.WriteLine($"располагает следующими тэгами: {print_iter(result.tags)}");
                            Console.WriteLine($"и актЄрами: {print_iter(result.actors)}");
                            Console.WriteLine($"режиссЄры - {print_iter(result.directors)}");
                        }
                        break;
                    case "b":
                        ...
                        {
                            Console.WriteLine("”казанный человек не найден");
                        }
                        else
                        {
                            int num = 1;
                            Console.WriteLine($"„еловек с именем {person_name} участвовал в следующих проектах:");
                    ...
                    case "c":
                        ...
                        {
                            Console.WriteLine("”казанный тэг не найден");
                        }
                        else
                        {
                            Console.WriteLine($"“эг {tag_name} присутствует в следующих фильмах:");
                            int num = 1;
